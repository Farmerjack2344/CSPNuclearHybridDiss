...
class AP1000Plant(ModelTemplate):
    def _create_network(self):
        ...
        try:
            ...
            print(f"Converged: {self.nw.status == 0} (status={self.nw.status})")

            if self.nw.status != 0:
                self.nw.print_structural_analysis()
                self.nw.print_variables()
                self.nw.print_incidence_matrix()
                self.nw.print_equations_with_dependents()
            else:
    ...
    def calc_efficiency(self):
        turbines = [
                    self.nw.get_comp('HP turbine'),
                    self.nw.get_comp('LP turbine stage 1'),
                    self.nw.get_comp('LP turbine stage 2'),
                    self.nw.get_comp('LP turbine stage 3')
                ]
        if self.nw.status == 0:
            turbine_power = sum(t.P.val_SI for t in turbines)
            pump_power = (self.nw.get_comp('condenser pump').P.val_SI +
                          self.nw.get_comp('HP pump').P.val_SI)
            net_power = -(turbine_power + pump_power)
            heat_input = self.nw.get_comp('steam generator').Q.val_SI
            thermal_eta = 100 * (net_power / heat_input)
...
if __name__ == "__main__":
    plant = AP1000Plant()
    eta = plant.get_parameter("efficiency")
    print(eta)
